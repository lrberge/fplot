...
Although the core of the package concerns distributions, `fplot` also offers functions to plot trends (possibly conditional), aggregate barplots and conditional boxplots. 
...
One last thing, what would happen if we add a x-axis label given that the full space is already taken by the journal labels? Let's try.
...
Everything fits! The tilt of the labels is increased and their size is made smaller to accomodate the x-axis label.
...
To display the umulative distribution, simply use the argument `cumul = TRUE`. Let's look at the cumulative distribution of the top 5% publications:

```{r}
plot_distr(institution ~ 1|jnl_top_5p, us_pub_econ, cumul = TRUE)
```

Just 10 institutions (the small black tick in between bars means 10) account for 33% of all top 5% publications.
